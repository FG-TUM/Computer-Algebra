...
main :: IO()
main = do
  print (quadraticSieve 87463 40 30)
  return ()

--------------------------------------------------------------
--quadratic Sieve (input, smoothness bound, sieving bound)----
--------------------------------------------------------------

--quadraticSieve :: Integer -> Integer -> Integer -> Maybe Integer
-- quadraticSieve x b s = quadraticSieveHelp x b s (filter (filterP x) (filter aksTest [1..s])) (map f1 [-s..(s-1)]) (newMatrix r)
quadraticSieve :: Integer -> Integer -> Integer -> [Integer]
quadraticSieve x b s = map f1 [-s..(s-1)]
      where f1 si = abs(pow(si + (sqr x))-x)
            pow x = x*x
            sqr x = sqrh x (div x 2)
            sqrh x s = if (pow s < x) then s else sqrh x (s-1)
            r = length (filter (filterP x) (filter aksTest [1..s]))
            newMatrix x = [ (newVect x) |z <- [0..x], x > z]
            newVect x = [0| z<-[0..x], x>z]

quadraticSieveHelp :: Integer -> Integer -> Integer -> [Integer] -> [Integer] -> [[Integer]] -> Maybe Integer
quadraticSieveHelp a b c d = undefined



filterP :: Integer -> Integer -> Bool
filterP n p = not(elem (inverse n p) xs)
  where xs = [toInteger (x*x)| x <- [1..10]]


--------------------------------------------------------------
--Test-for-perfect-Power+Helpers------------------------------
--------------------------------------------------------------

tfpp :: Integer -> Maybe Integer
tfpp n = extract $ map (tfppH 2 n n) [2 .. floor $ logBase 2 (fromInteger n)]
...
tfppH :: Integer -> Integer -> Integer -> Integer -> Maybe Integer
tfppH m1 m2 n e
  | m1 > m2   = Nothing
  | mE == n   = Just m
  | mE > n    = tfppH m1 (m-1) n e
  | otherwise = tfppH (m+1) m2 n e
  where m = div (m1 + m2) 2
        mE = fastExp m e
...
aksTest n
  ...
  | tfpp n /= Nothing = False  -- not Nothing => n is a perfect power
  | r == n                        = True
  | mod n r == 0 && r < n         = False
...
inverse a b = forcePositive $ extract (eea a b)
  where extract (_,c,_) = c
        forcePositive c | c < 0 = c + b
                        |otherwise = c
